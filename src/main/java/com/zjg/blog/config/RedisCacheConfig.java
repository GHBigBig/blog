...
public class RedisCacheConfig extends CachingConfigurerSupport {
    // spring 装载的 redis 连接工厂
    @Autowired
    private RedisConnectionFactory redisConnectionFactory;

    //自定义的用来读取 yml 文件中每个缓存名对应的缓存过期时间的属性类
    @Autowired
    ...
    /*
        ...
        就让其使用默认的 KeyGeneretor 就好， 一个 service 中我们规定只操作一种类型的数据，默认的报
            ClassCastException: org.springframework.cache.interceptor.SimpleKey
            cannot be cast to java.lang.String
        将默认的 SimpleKey 改为 String
     */

    /**
     * 自定义缓存中 key 的生成规则，若传入的参数仅为实体的类的 id
     *  且无其他类型可自动生成否则就要手动指定，规则为：
     *      空参：         类名
     *      1个参数：      类名 + 空格 + 参数
 *      2个参数及以上：    类名 + 空格 + 【 + 集合里的id（中间空格相隔） + 】
     *
     * @return key name
     */
    @Bean
    ...
    /*
        配置 RedisTemplate
        是为了替换默认的 JDK 序列化器，使用默认的序列化器不便于阅读

        此处在 SpringBoot 中的实现是，有一个默认的 的RedisTemplate Bean,但使用了
    ...
    public RedisTemplate redisTemplate() {
        ...
        //InitializingBean 接口提供的一个方法，在 spring 容器属性初始化完成后再调用该方法
    ...
    /*
        创建缓存管理器
        主要为了自定义若干 cacheNames 和缓存时间；

        自定义该类后，如果缓存注解中使用了一个未配置的缓存名，并且该类的一个 dynamic 属性为 true
    ...
    public CacheManager cacheManager() {
        ...
        //默认的过期时间，会被每个缓存自己的过期时间覆盖
        redisCacheManager.setDefaultExpiration(3600);
        /*
            启动是加载远程缓存，不开启：每次第一次查询即使缓存中已经有了旧的缓存,也不会读取到;
        ...
        Map<String, Long> expires = customRedisCacheExpireProperties.generateExireMap();
        //设置每个缓存对应的过期时间
